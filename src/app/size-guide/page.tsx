...
function SizeGuideContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/products/${productId}`, { cache: 'no-store' })
      .then((r) => r.json())
      .then((data: { product?: StoreProduct }) => {
  ...
  const hasCustomChart = product && ((product.sizeChartImageUrl && product.sizeChartImageUrl.trim()) || (product.sizeChartRows && product.sizeChartRows.length > 0));
  ...
  return (
    <div className="pt-32 pb-24 bg-cream min-h-screen">
      <div className="max-w-4xl mx-auto px-6">
        ...
        {/* Fallback generic table if no custom chart */}
        {!loading && !hasCustomChart && (
        ...
        {!loading && hasCustomChart && product && (
          <div className="flex flex-col gap-16">
            <motion.div
              id={`chart-${product.productId}`}
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-playfair text-brown" style={{ fontFamily: "'Playfair Display', serif" }}>{product.name}</h2>
              ...
              {!product.sizeChartImageUrl && product.sizeChartRows && product.sizeChartRows.length > 0 && (
                <div className="rounded-2xl overflow-hidden border border-nude/30 shadow-xl shadow-brown/5 mb-8 bg-white">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody>
                        {product.sizeChartRows.map((row, ri) => (
              ...
              <div className={`grid grid-cols-1 gap-4 ${getMeasureTips(product.sizeChartRows).length <= 2 ? 'sm:grid-cols-2' : getMeasureTips(product.sizeChartRows).length === 3 ? 'sm:grid-cols-3' : 'sm:grid-cols-2 lg:grid-cols-4'}`}>
                {getMeasureTips(product.sizeChartRows).map(([key, tip]) => (
                  <div
                    key={key}
